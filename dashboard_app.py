...
with tab4:
    ...
    st.caption("Ingresa las características de un juego y el modelo predice sus ventas globales.")
 
    # ── opciones para los selectbox ───────────────────────────────────────────
    # se extraen del dataset original (no del filtrado) para tener todas las categorías
    # que el encoder conoce; si se usara el filtrado, podrían faltar categorías
    opciones_platform                  = sorted(videojuegos['platform'].dropna().unique())
    opciones_genre                     = sorted(videojuegos['genre'].dropna().unique())
    opciones_rating_esrb               = sorted(videojuegos['rating_esrb'].dropna().unique())
    opciones_gen_platform              = sorted(videojuegos['gen_platform'].dropna().unique())
    opciones_classification_user_score = sorted(videojuegos['classification_user_score'].dropna().unique())
 
    # rangos para los inputs numéricos derivados del dataset completo
    anio_min_pred  = int(videojuegos['year_of_release'].min())
    anio_max_pred  = int(videojuegos['year_of_release'].max())
    critic_min     = float(videojuegos['critic_score'].min())
    critic_max     = float(videojuegos['critic_score'].max())
    user_min       = float(videojuegos['user_score'].min())
    user_max       = float(videojuegos['user_score'].max())
 
    # ── layout: dos columnas para los widgets ─────────────────────────────────
    col_izq, col_der = st.columns(2)
 
    with col_izq:
        # st.selectbox: menú desplegable de selección única
        # las opciones vienen del CSV; no hay valores hardcodeados
        inp_platform = st.selectbox(
            "Plataforma",
            options=opciones_platform
        )
        inp_genre = st.selectbox(
            "Género",
            options=opciones_genre
        )
        inp_rating_esrb = st.selectbox(
            "Rating ESRB",
            options=opciones_rating_esrb
        )
        inp_gen_platform = st.selectbox(
            "Generación de plataforma",
            options=opciones_gen_platform
        )
        inp_classification_user_score = st.selectbox(
            "Clasificación user score",
            options=opciones_classification_user_score
        )
 
    with col_der:
        # st.number_input: campo numérico con límites y paso definidos
        # min_value / max_value: cotas derivadas del dataset; evitan valores imposibles
        # value: valor por defecto al cargar la pestaña
        inp_year = st.number_input(
            "Año de lanzamiento",
            min_value=anio_min_pred,
            max_value=anio_max_pred,
            value=2010,
            step=1
        )
        inp_critic_score = st.number_input(
            "Critic Score",
            min_value=critic_min,
            max_value=critic_max,
            value=round((critic_min + critic_max) / 2, 1),
            step=0.5,
            help="Puntuación de la crítica especializada (escala del dataset)"
        )
        inp_user_score = st.number_input(
            "User Score",
            min_value=user_min,
            max_value=user_max,
            value=round((user_min + user_max) / 2, 1),
            step=0.1,
            help="Puntuación de usuarios (escala del dataset)"
        )
    ...
    if st.button("Predecir ventas", type="primary"):
        ...
        # armar el payload con exactamente los mismos nombres de campo que JuegoInput
        # si algún nombre difiere, FastAPI devuelve un error 422 (unprocessable entity)
        payload = {
            "platform":                  inp_platform,
            "genre":                     inp_genre,
            "rating_esrb":               inp_rating_esrb,
            "gen_platform":              inp_gen_platform,
            "classification_user_score": inp_classification_user_score,
            "year_of_release":           int(inp_year),
            "critic_score":              float(inp_critic_score),
            "user_score":                float(inp_user_score)
        }
        ...
        try:
            ...
            # timeout=10: si la api no responde en 10 segundos, lanza una excepción
            respuesta = requests.post(API_URL, json=payload, timeout=10)
 
            if respuesta.status_code == 200:
                # extraer el valor de prediccion_ventas del JSON de respuesta
                prediccion = respuesta.json()["prediccion_ventas"]
 
                # st.metric: muestra el resultado con formato destacado
                st.metric(
                    label="Ventas globales estimadas",
                    value=f"{prediccion:.2f} M"
                )
                st.success("Predicción generada correctamente.")
 
            else:
                # status_code distinto de 200: la api respondió pero con un error
                # mostrar el código y el cuerpo para facilitar el diagnóstico
                st.error(
                    f"La API respondió con el código {respuesta.status_code}. "
